...
mod tests {
    ...
    #[test]
    fn test_buffered_stream_context_updates_message_start_input_tokens() {
        use crate::kiro::model::events::ContextUsageEvent;

        let mut ctx = BufferedStreamContext::new("test-model", 123, false);
        ...
        let msg_start = events.iter().find(|e| e.event == "message_start").unwrap();
        let input_tokens = msg_start.data["message"]["usage"]["input_tokens"]
            .as_i64()
            .unwrap();
        assert_eq!(input_tokens, 20000);
    }
