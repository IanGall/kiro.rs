...
mod tests {
    ...
    #[test]
    fn test_usage_with_bonus_and_trial() {
        let json = r#"{
          "usageBreakdownList": [{
            ...
            "bonuses": [
              ...
              { "currentUsage": 1, "usageLimit": 5, "status": "EXPIRED" }
            ],
            "freeTrialInfo": {
        ...
        assert_eq!(resp.current_usage(), 10.0 + 3.0 + 2.0);
        assert_eq!(resp.usage_limit(), 100.0 + 30.0 + 20.0);
    }

    #[test]
    fn test_usage_without_bonus() {
        let json = r#"{
          "usageBreakdownList": [{
        ...
        let resp: UsageLimitsResponse = serde_json::from_str(json).unwrap();
        assert_eq!(resp.current_usage(), 5.0);
        assert_eq!(resp.usage_limit(), 50.0);
    }
}
