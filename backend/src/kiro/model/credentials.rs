...
pub struct KiroCredentials {
    ...
    /// 认证方式 (social / idc / builder-id / iam)
    #[serde(skip_serializing_if = "Option::is_none")]
...
mod tests {
    ...
    #[test]
    fn test_canonicalize_auth_method() {
        let mut creds = KiroCredentials::default();
        creds.auth_method = Some("builder-id".to_string());
        creds.canonicalize_auth_method();
        assert_eq!(creds.auth_method, Some("idc".to_string()));

        creds.auth_method = Some("iam".to_string());
        creds.canonicalize_auth_method();
        assert_eq!(creds.auth_method, Some("idc".to_string()));
